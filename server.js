...
app.delete('/api/products/:id', async (req, res) => {
  ...
  // Endpoint para atualizar o status das transações pendentes
app.get('/api/update-status', async (req, res) => {
  try {
    // Busca todas as transações com status "Gerado"
    const pendingTransactions = await PurchaseHistory.findAll({
      where: { status: 'Gerado' }
    });
    
    // Para cada transação pendente, consulta a API externa para verificar o status
    for (const transaction of pendingTransactions) {
      try {
        ...
        // Se o status retornado não for "created", atualiza a transação para "Sucesso"
        if (data.status && data.status.toLowerCase() !== 'created') {
          await PurchaseHistory.update(
            { status: 'Sucesso' },
